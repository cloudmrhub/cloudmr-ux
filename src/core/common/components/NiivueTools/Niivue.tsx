...
export default function NiiVueport(props: {
  ...
  setWarning: Function;
  setWarningOpen: Function;
  pipelineID: string;
  saveROICallback: Function;
  rois: any[];
  niis: any[];
  ...
  const [drawingEnabled, setDrawingEnabled] = React.useState(
  ...
  const lastMMRef = React.useRef<number[] | null>(null);
  ...
  const [rois, setROIs] = React.useState<ROI[]>([]);
  ...
  function verifyComplex(volume: any) {
  ...
  function nvSetDisplayedVoxels(voxelType: string) {
  ...
  const layerList = layers.map((layer: any, index: number) => {
    return index === 0 ? ( //Yuelong: we shall expect only one effective layer in this implementation
      <Layer
        ...
        getColorMapValues={(colorMapName: string) => {
  ...
  async function addLayer(file: any) {
  ...
  function nvUpdateDrawingOpacity(opacity: number) {
  ...
  function nvUpdateOpacity(a: number) {
  ...
  function nvSetDragMode(dragMode: string) {
    switch (dragMode) {
      case "none":
  ...
  function nvSetDrawingEnabled(enabled: boolean) {
    setDrawingEnabled(enabled);
    nv.setDrawingEnabled(enabled);
    nv.drawScene();
  }

  function nvUpdateDrawPen(a: any) {
  ...
  function nvUpdateBrushSize(size: number) {
    setBrushSize(size);
    nv.opts.penBounds = (size - 1) / 2;
  }

  function nvUpdateDrawOpacity(a: number) {
    setDrawOpacity(a);
    nv.setDrawOpacity(a);
  }

  function nvUpdateCrosshairColor(rgb01: number[], a = 1) {
  ...
  function nvUpdateMultiplanarPadPixels(v: any) {
  ...
  function nvUpdateRulerWidth(v: number) {
    nv.opts.rulerWidth = v;
    setRulerWidth(v);
    nv.drawScene();
  }

  function nvUpdateRulerOpacity(a: number) {
    nv.opts.rulerColor = [rulerColor[0], rulerColor[1], rulerColor[2], a];
    setRulerOpacity(a);
    nv.drawScene();
  }

  function nvUpdateLongTouchTimeout(v: number) {
    nv.opts.longTouchTimeout = v;
    setLongTouchTimeout(v);
  }

  function nvUpdateDoubleTouchTimeout(v: number) {
  ...
  function nvUpdateMaxDrawUndoBitmaps(v: number) {
    nv.opts.maxDrawUndoBitmaps = v;
    setMaxDrawUndoBitmaps(v);
  }

  function nvUpdateBackColor(rgb01: number[], a = 1) {
    setBackColor([...rgb01, a]);
    nv.opts.backColor = [...rgb01, a];
    nv.drawScene();
  }

  function nvUpdateRulerColor(rgb01: number[], a = 1) {
  ...
  function nvUpdateClipPlaneColor(rgb01: number[], a = 1) {
  ...
  function nvUpdateTextSize(v: number) {
    setTextSize(v);
    nv.opts.textHeight = v;
    nv.drawScene();
  }

  function updateDecimalPrecision(v: number) {
  ...
  function nvUpdateCrosshairOpacity(a: number) {
  ...
  function nvUpdateClipPlaneOpacity(a: number) {
    nv.opts.clipPlaneColor = [
      clipPlaneColor[0],
      clipPlaneColor[1],
      clipPlaneColor[2],
  ...
  function nvUpdateCrosshairSize(w: number) {
  ...
  const [labelMapping, setLabelMapping] = useState<LabelMap>({});
  function resampleImage(mapping: LabelMap = labelMapping) {
    let image = nv.volumes[0];
    let rois: ROI[] = [];
    let layout = {
      barmode: "overlay" as const,
      title: { text: "ROI Histogram" }, // Set your title here
      // height: 100,
      margin: {
        l: 50, // left margin
        r: 50, // right margin
    ...
    for (let i = 0; i < nv.drawBitmap.length; i++) {
      ...
      if (samples[nv.drawBitmap[i] as number] === undefined) {
    ...
    Object.keys(samples).forEach((_key) => {
  ...
  function nvUpdateLayerOpacity(a: number) {
    nv.updateGLVolume();
  }

  function nvUpdateColorMap(id: any, clr: any) {
  ...
  const selectVolume = async (volumeIndex: any) => {
    const openVolume = async () => {
      nv.closeDrawing();
      setDrawingChanged(false);
      if (drawingEnabled) nvUpdateDrawingEnabled();
  ...
  const [selectedROI, setSelectedDrawingLayer] = useState<number | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  type strCallback = (text: string) => void;
  const [saveConfirmCallback, setSaveConfirmCallback] = useState<strCallback>(
    (text: string) => {},
  );

  type voidCallback = () => void;
  const [confirmationOpen, setConfirmationOpen] = useState(false);
  const [warningConfirmationCallback, setWarningConfirmationCallback] =
    useState<voidCallback>(() => {});
  const [warningCancelCallback, setWarningCancelCallback] =
    useState<voidCallback>(() => {});
  const [drawingChanged, setDrawingChanged] = useState(false);

  const setLabelAlias = function (label: string | number, alias: string) {
    labelMapping[label] = alias;
    setLabelMapping(labelMapping);
    resampleImage(labelMapping);
  };
  const zipAndSendDrawingLayer = async function (
    uploadURL: string,
    filename: string,
    blob: Blob,
  ) {
  ...
  const unzipAndRenderDrawingLayer = async (
    accessURL: string,
    ...
    if (fileInfo) {
      ...
      if (niiDrawing) {
        ...
        if (
          mergeIntoExisting &&
          typeof (nv as any).mergeDrawingFromBase64 === "function"
        ) {
          const mergeResult = await (nv as any).mergeDrawingFromBase64(
            niiFilePath,
            base64,
          );
          if (!mergeResult || !mergeResult.ok) {
            props.warn(
              "Failed to merge ROI upload (invalid file or size mismatch).",
            );
            return null;
          }
          if (mergeResult.importLabelRemap === null) {
            ...
            const merged = { ...labelMapping } as Record<string, string>;
            for (const [oldL, newId] of Object.entries(
              mergeResult.importLabelRemap,
            )) {
              ...
              const alias =
                oldN === newN
                  ? (importedLabels[oldL] ??
                    ...
                    String(newId))
                  : String(newId);
              merged[String(newId)] = alias as string;
            }
  ...
  const selectDrawingLayer = async (roiIndex: number) => {
  ...
  const unpackROI = async (accessURL: string) => {
  ...
  const refreshROI = async () => {
    ...
    if (roiIndex !== null) {
      const load = (idx: number) => {
        ...
        console.trace();
        nv.loadDrawingFromUrl(props.rois[idx].link).then((value: any) => {
          resampleImage();
        });
        setSelectedDrawingLayer(roiIndex);
        setDrawingChanged(false);
  ...
  const saveDrawingLayer = (
    ...
    setSaveConfirmCallback(() => async (filename: string) => {
      preSaveCallback();
      const config = {};
    ...
    >
      ...
      >
        ...
        <select
          name="drawPen"
          id="drawPen"
          onChange={nvUpdateDrawPen}
          defaultValue={drawPen}
        >
        ...
        <Button title={"Save image"} onClick={nvSaveImage}>
          Save image
        </Button>
        <NVSwitch
      ...
      <Confirmation
        ...
        cancelCallback={warningCancelCallback}
...
function niiToVolume(nii: { link: string; filename: string; name: string }) {
  return {
