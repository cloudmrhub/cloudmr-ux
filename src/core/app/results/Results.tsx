...
import {
  getPipelineROI,
  loadResult,
} from "../../features/rois/resultActionCreation";
...
const Results = ({
  ...
  const { queueToken } = useAppSelector((state) => state.authenticate);
  const results = useAppSelector((state) => state.jobs.jobs);
  ...
  const activeJobAlias = useAppSelector(
    (state) => state.result.activeJob?.alias,
  );
  const pipelineID = useAppSelector(
            ...
            <Tooltip title={`Delete job ${params.row.alias}`}>
              <IconButton
                onClick={() => {
                  ...
                  setMessage(
                    `Please confirm that you are deleting job ${params.row.id.toString()}.`,
                  );
                  setColor("error");
                  setConfirmCallbackjob(() => async () => {
                    try {
                      ...
                      console.error(
                        "Error deleting job or fetching updated jobs:",
                        error,
                      );
                    }
                  });
        ...
        <CmrPanel header="Job Results" className={"mb-2"} key={"0"}>
          ...
          >
            ...
            {jobsLoading && (
              <CircularProgress
                size={18}
                style={{ color: "white", position: "relative", left: "5pt" }}
              />
            )}
          </Button>
          {showingLogs && <Logs />}
