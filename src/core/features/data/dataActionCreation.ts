...
export const getUploadedData = createAsyncThunk('GET_UPLOADED_DATA',
    async () => {
        const endpoints = getEndpoints();
        const config = getAppConfig();
        try {
            const { data } = await AuthenticatedHttpClient.get(endpoints.DATA_API);
            return data;
        } catch (e) {
            console.log(e);
            return undefined;
...
export const renameUploadedData = createAsyncThunk('RENAME_UPLOADED_DATA',
    async ({ fileId, newName }: { fileId: number, newName: string }) => {
        const endpoints = getEndpoints();
        
        try {
            const response = await AuthenticatedHttpClient.post(endpoints.DATA_RENAME_API, {
                fileId,
                newName
            });
...
export const deleteUploadedData = createAsyncThunk('DELETE_UPLOADED_DATA',
    async ({ fileId }: { fileId: number, accessToken: string }) => {
        const endpoints = getEndpoints();
        
        try {
            const response = await AuthenticatedHttpClient.post(`${endpoints.DATA_DELETE_API}/${fileId}`);
            return response.data;
        } catch (e) {
            throw e;
        }
...
export const uploadData = createAsyncThunk('UploadData', async (
    { uploadToken, file, fileAlias, onProgress, onUploaded, uploadTarget }:
        {
    ...
    try {
        ...
        const uploadPartWithRetries = async (partUrl: string,
            part: any, cancelTokenSource: any,
            index: number, retries = 2): Promise<any> => {
            try {
        ...
        const endpoints = getEndpoints();
        const finalizeResponse = await AuthenticatedHttpClient.post(endpoints.DATA_UPLOAD_FINALIZE, {
            uploadId,
            parts: uploadedParts,
            Key: Key
        }, payload.config);
...
const ALLOWED_EXTENSIONS = [
    'nii', 'nii.gz', 'mha', 'mhd', 'mrd',
    'npx', 'npy', 'pkl', 'mat', 'dat', 'h5',
    'png', 'jpg', 'jpeg'
];
