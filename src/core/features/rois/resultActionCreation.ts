...
export const getPipelineROI = createAsyncThunk('GetROI', async ({pipeline}:{pipeline:string}) => {
    ...
    const response = await AuthenticatedHttpClient.get(endpoints.ROI_GET, config);
...
export const loadResult = createAsyncThunk('LoadResult', async ({job}:{accessToken:string,job:Job})=>{
    ...
    // console.log(file);
    let result = (await axios.post(endpoints.UNZIP, JSON.parse(file.location),{
    })).data;
    // console.log(result);
